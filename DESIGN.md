...
Family-level design description of the four OpenRX variants. Values are extracted from the KiCad design files and the ExpressLRS target JSON in `shared/elrs-targets/`. Per-variant RF-chain detail: `OpenRX-<variant>/DESIGN.md`. Flashing, debug, and RF-switch state decode: [FLASHING.md](FLASHING.md).
...
- **Flashing**: UART, Wi-Fi OTA, or Betaflight passthrough. Procedures in [FLASHING.md](FLASHING.md).
